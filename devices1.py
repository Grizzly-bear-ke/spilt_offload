...
import pickle
import pathlib
from ecci_sdk_edge_1 import Client
import multiprocessing as mp
import threading, random, socket, socketserver, time, pickle
from datasets_data.vid_dataset import ImagenetDataset
from datasets_data.data_preprocessing import group_annotation_by_class

parser = argparse.ArgumentParser(description='tracking demo')
parser.add_argument("--dataset", type=str, default="G:/baidupandownload/kubeedge_meeting/dataset/imagenet2015/ILSVRC/",
...
# Initialize ecci sdk and connect to the broker in edge-cloud
    ...
    for f in range(len(dataset)):
        ...
        if data_flag == 'start':
            ...
            for i in range(bz_num):
                ...
                frame = cv2.resize(frame, (224, 224), fx=1.2, fy=1.6, interpolation=cv2.INTER_CUBIC)
            ...
            # jpg_as_time = base64.b64encode(stat_time)
            print("jpg_as_text")


            # send frame to cloud
            payload = {"type": "data", "contents": {"frame1": jpg_as_text, "time1": stat_time, "bz": bz_num}}
            #print("####################", payload)
            ecci_client.publish(payload, "cloud1")
    ...
    payload = {"type": "data", "contents": {"frame1": "end"}}
    # print("####################", payload)
    ecci_client.publish(payload, "cloud1")
