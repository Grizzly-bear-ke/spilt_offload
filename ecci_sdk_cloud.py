...
PUB_TARGETS = ast.literal_eval(os.getenv("ECCI_PUB_TARGETS",default = "{'edge': 'RANDOMSTRING'}" ))

BROKER_IP = os.getenv("ECCI_LOCAL_BROKER_IP", default= "192.168.3.6")
BROKER_PORT = int(os.getenv("ECCI_LOCAL_BROKER_PORT", default="1883"))
BROKER_ID = os.getenv("ECCI_LOCAL_BROKER_ID", default= "RANDOMSTRING")
BROKER_USERNAME = os.getenv("ECCI_LOCAL_BROKER_USERNAME")
BROKER_PASSWORD = os.getenv("ECCI_LOCAL_BROKER_PASSWORD")

APP_ID = os.getenv("ECCI_APP_ID", default= "RANDOMSTRING")
# CONTAINER_NAME = re.findall(r"^edgeai_.*?_(.*)", os.getenv("ECCI_CONTAINER_NAME"))[0]
CONTAINER_NAME = os.getenv("ECCI_CONTAINER_NAME", default="cloud")
BRIDGE_MOUNTPOINTS = ast.literal_eval(os.getenv("ECCI_BRIDGE_MOUNTPOINTS",default="[]"))

# ---------------------------任务3-4新增---------------------------
ECCI_APP_TYPE = os.getenv("ECCI_APP_TYPE", default="component")  #controller/component
# 发送就绪命令给controller的target_name:相应的broker_id PS：len()==1/0
ECCI_APP_CONTROLLER_CONTAINER = ast.literal_eval(os.getenv("ECCI_APP_CONTROLLER_CONTAINER",default="{'edge': 'RANDOMSTRING'}"))
# 在label策略下存在，表示pub_targets中同镜像不同容器名的种类，可用于选择某类应用component中的部分容器
ECCI_APP_PUB_TARGETS_GROUPS = ast.literal_eval(os.getenv("ECCI_APP_CONTROLLER_CONTAINER",default="{'edge': 'RANDOMSTRING'}"))
...
CONTAINER_TYPE = os.getenv("ECCI_CONTAINER_TYPE", default="cloud")
        ...
        if sender in LISTEN_CONTAINERS:
            ...
            if all(list(LISTEN_CONTAINERS.values())):
                ...
                if ECCI_APP_CONTROLLER_CONTAINER:
                    ...
                    payload = {"app_id": APP_ID, "container_name": CONTAINER_NAME, "agent_id": ECCI_AGENT_ID,
                               "ready_type": "app"}
        ...
        try:
            if targets == None:
                ...
                for target_item in target_list:
                    self._mqtt_client.publish(topic=PUB_TOPICS[target_item], payload=cPickle.dumps(message), qos=2,
                                              retain=retain)
            else:
